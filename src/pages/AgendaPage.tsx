...
export function AgendaPage({ meliponaries, colonies, boxes, activeMeliponaryId }: Props) {
  ...
  useEffect(() => { void reload(); }, [view, taskType, priority, colonyId, search, activeMeliponaryId]);
  ...
  async function reload() {
    ...
    try {
      ...
      setError(messageFrom(cause, "Não foi possível carregar a Agenda."));
    } finally {
      setLoading(false);
    }
  }
  ...
  async function mutate(action: () => Promise<unknown>) {
    ...
    try {
      ...
      setError(messageFrom(cause, "Não foi possível concluir a ação da Agenda."));
      return false;
    } finally {
      setBusy(false);
    }
  ...
  function openCreate() {
    setCreateForm({ meliponaryId: defaultMeliponaryId, colonyId: "", boxId: "", taskType: "generic", title: "", description: "", scheduledFor: "", priority: "normal" });
    setCreateOpen(true);
  }
  ...
  return <div className="page-stack">
    ...
    <section className="panel wide-list">
      ...
      {loading ? <div className="empty-list">Carregando Agenda...</div> : items.length === 0 ? <div className="empty-list">Nenhuma tarefa nesta visualização.</div> : <div className="table-wrap"><table className="data-table"><thead><tr><th>Quando</th><th>Tarefa</th><th>Contexto</th><th>Tipo</th><th>Prioridade</th><th>Estado</th><th>Ações</th></tr></thead><tbody>{items.map((task) => <tr key={task.id} className={task.overdue ? "attention-row" : undefined}><td><strong>{formatDateTime(task.scheduledFor)}</strong>{task.overdue && <small className="cell-note">Atrasada</small>}</td><td><strong>{task.title}</strong>{task.description && <small className="cell-note">{task.description}</small>}</td><td>{task.colonyCode || task.boxCode || task.meliponaryName}</td><td>{taskTypeLabel(task.taskType)}</td><td><span className={`badge severity-${task.priority}`}>{priorityLabel(task.priority)}</span></td><td><span className={`badge task-${task.status}`}>{statusLabel(task.status)}</span></td><td><RecordActions busy={busy} onOpen={() => setDetail(task)} secondary={task.status === "pending" ? [
    ...
    <Dialog open={Boolean(detail)} onClose={() => setDetail(null)} title="Tarefa" description={detail ? detail.title : ""} size="medium">{detail && <div className="detail-grid"><div><span>Quando</span><strong>{formatDateTime(detail.scheduledFor)}</strong></div><div><span>Estado</span><strong>{statusLabel(detail.status)}</strong></div><div><span>Tipo</span><strong>{taskTypeLabel(detail.taskType)}</strong></div><div><span>Prioridade</span><strong>{priorityLabel(detail.priority)}</strong></div><div><span>Meliponário</span><strong>{detail.meliponaryName}</strong></div><div><span>Contexto</span><strong>{detail.colonyCode || detail.boxCode || "Administrativa"}</strong></div><div className="full"><span>Descrição</span><p>{detail.description || "—"}</p></div>{detail.completedById && <div className="full"><span>Fato vinculado</span><p>{detail.completedByType}: {detail.completedById}</p></div>}{detail.cancellationReason && <div className="full"><span>Motivo do cancelamento</span><p>{detail.cancellationReason}</p></div>}{detail.skipReason && <div className="full"><span>Motivo para ignorar</span><p>{detail.skipReason}</p></div>}</div>}</Dialog>
...
function messageFrom(cause: unknown, fallback: string) { const text = String(cause ?? "").replace(/^Error:\s*/, "").trim(); return text || fallback; }
