...
export function FeedingPage({ colonies, busy, recordStateMap, autoCreate = false, onCreate, onCorrect, onVoid }: Props) {
  ...
  return <div className="page-stack">
    ...
    {colonies.length === 0 ? <div className="inline-notice" role="status">Cadastre uma colônia antes de registrar alimentação.</div> : <section className="panel wide-list"><div className="panel-heading"><h2>Histórico da colônia</h2><p>Correções preservam a versão anterior na auditoria; anulações continuam visíveis e deixam de gerar pendências.</p></div>{!selectedColonyId ? <div className="empty-list">Selecione uma colônia na toolbar para consultar o histórico.</div> : loading ? <div className="empty-list" role="status">Carregando alimentações...</div> : loadError ? <div className="inline-notice" role="alert">{loadError}</div> : items.length === 0 ? <div className="empty-list">Nenhuma alimentação registrada para esta colônia.</div> : <div className="table-wrap"><table className="data-table"><thead><tr><th>Data</th><th>Alimento</th><th>Quantidade</th><th>Caixa</th><th>Próxima</th><th>Estado</th><th>Ações</th></tr></thead><tbody>{items.map((item) => { const state = recordStateMap.get(`feeding:${item.id}`); return <tr key={item.id} className={state?.voidedAt ? "voided-row" : undefined}><td><strong>{formatDateTimeBr(item.fedAt)}</strong></td><td>{item.foodType}</td><td>{quantityLabel(item)}</td><td>{item.boxCode || "Sem caixa"}</td><td>{item.nextFeedingAt ? formatDateTimeBr(item.nextFeedingAt) : "Sem agendamento"}</td><td><RecordStateBadge state={state} /></td><td><RecordActions busy={busy} onOpen={() => setDetail(item)} onEdit={state?.voidedAt ? undefined : () => beginCorrect(item)} secondary={[{ label: "Anular", onClick: () => setVoidTarget(item), disabled: Boolean(state?.voidedAt), danger: true }]} /></td></tr>; })}</tbody></table></div>}</section>}
    ...
    <Dialog open={Boolean(detail)} onClose={() => setDetail(null)} title="Alimentação" description={detail ? `${detail.colonyCode} · ${formatDateTimeBr(detail.fedAt)}` : ""} size="medium">{detail && <div className="detail-grid"><div><span>Caixa</span><strong>{detail.boxCode || "Sem caixa"}</strong></div><div><span>Alimento</span><strong>{detail.foodType}</strong></div><div><span>Quantidade</span><strong>{quantityLabel(detail)}</strong></div><div><span>Próxima</span><strong>{detail.nextFeedingAt ? formatDateTimeBr(detail.nextFeedingAt) : "Sem agendamento"}</strong></div><div className="full"><span>Resposta observada</span><p>{detail.responseNotes || "—"}</p></div><div className="full"><span>Observações</span><p>{detail.notes || "—"}</p></div></div>}</Dialog>
