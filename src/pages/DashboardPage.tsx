...
export function DashboardPage({ stats, data, activeMeliponaryId, onNavigate }: DashboardPageProps) {
  ...
  const inspectionIntent: NavigationIntent = { view: "inspections", meliponaryId: activeMeliponaryId || undefined, colonyId: scopedColonies.length === 1 ? scopedColonies[0].id : undefined, action: "create" };
  ...
  return <div className="page-stack">
    <PageToolbar title="Visão geral" description={activeMeliponaryId ? "Situação operacional do meliponário selecionado." : "Situação operacional consolidada de todos os meliponários."}>
      ...
      <button type="button" className="button-secondary" onClick={() => onNavigate(inspectionIntent)}>Nova inspeção</button>
    </PageToolbar>
    ...
    <section className="panel wide-list">
      ...
      {scopedAlerts.length === 0 ? <div className="empty-list">Nenhuma pendência atual.</div> : <div className="table-wrap"><table className="data-table"><thead><tr><th>Contexto</th><th>Alerta</th><th>Quando</th><th>Severidade</th><th>Ação</th></tr></thead><tbody>{scopedAlerts.slice(0, 8).map((alert) => <tr key={alert.alertKey}><td><strong>{alert.colonyCode || alert.boxCode || "Meliponário"}</strong></td><td>{alert.title}</td><td>{alert.dueAt ? formatDateTimeBr(alert.dueAt) : alertTypeLabel(alert.alertType)}</td><td><span className={`badge severity-${alert.severity}`}>{severityLabel(alert.severity)}</span></td><td><button className="button-secondary" type="button" onClick={() => onNavigate(alert.taskId ? { view: "agenda", taskId: alert.taskId, colonyId: alert.colonyId, boxId: alert.boxId, meliponaryId: alert.meliponaryId } : recommendedIntent(alert))}>{alert.taskId ? "Abrir tarefa" : recommendedLabel(alert.recommendedAction)}</button></td></tr>)}</tbody></table></div>}
...
function recommendedIntent(alert: Alert): NavigationIntent { return { view: recommendedView(alert.recommendedAction), colonyId: alert.colonyId, boxId: alert.boxId, meliponaryId: alert.meliponaryId, action: "create" }; }
