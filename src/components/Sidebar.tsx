...
const items: Array<{ view: View; label: string; short: string }> = [
  ...
  { view: "alerts", label: "Alertas", short: "AT" },
  { view: "genealogy", label: "Divisões", short: "DV" },
  { view: "movements", label: "Movimentações", short: "MV" },
];
  ...
  return (
    <aside className="sidebar">
      ...
      <nav className="side-nav" aria-label="Navegação principal">
        {items.map((item) => <button className={activeView === item.view ? "nav-item active" : "nav-item"} key={item.view} onClick={() => onNavigate(item.view)} aria-current={activeView === item.view ? "page" : undefined} type="button"><span className="nav-short">{item.short}</span><span>{item.label}</span></button>)}
      </nav>
