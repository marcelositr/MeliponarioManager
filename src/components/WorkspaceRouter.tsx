...
export function WorkspaceRouter({ activeView, navigationIntent, activeMeliponaryId, data, stats, busy, actions, recordStateMap, onNavigate }: WorkspaceRouterProps) {
  ...
  if (activeView === "species") return <SpeciesPage items={data.species} busy={busy} onCreate={actions.createSpecies} onImport={actions.importSpeciesCsv} onEdit={actions.editSpecies} onArchive={actions.archiveSpecies} onReactivate={actions.reactivateSpecies} onDelete={actions.deleteSpecies} />;
