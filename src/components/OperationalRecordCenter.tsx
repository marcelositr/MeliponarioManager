...
export function ColonyOperationalCenter({ colonyId, onNavigate }: NavigateProps & { colonyId: string }) {
  ...
  if (error) return <div className="inline-notice">{error}</div>;
  if (!center) return <div className="empty-list">Carregando ficha operacional...</div>;
  ...
  return <div className="page-stack compact-stack">
    <section className="panel">
      ...
      <div className="summary-grid">
        ...
        <Metric label="Última inspeção" value={center.latestInspectionAt ? formatDateTime(center.latestInspectionAt) : "Sem inspeção"} />
        <Metric label="Força observada" value={strengthLabel(center.latestStrength)} />
        <Metric label="Última alimentação" value={center.latestFeedingAt ? formatDateTime(center.latestFeedingAt) : "Sem alimentação"} />
        ...
        <Metric label="Próximo compromisso" value={center.nextTaskTitle ? `${center.nextTaskTitle}${center.nextTaskAt ? ` · ${formatDateTime(center.nextTaskAt)}` : ""}` : "Nenhum"} />
      </div>
      <div className="workspace-actions">
        <button type="button" onClick={() => onNavigate("agenda")}>Abrir Agenda</button>
        <button className="button-secondary" type="button" onClick={() => onNavigate("inspections")}>Inspeções</button>
        <button className="button-secondary" type="button" onClick={() => onNavigate("feeding")}>Alimentação</button>
        <button className="button-secondary" type="button" onClick={() => onNavigate("history")}>Histórico</button>
        <button className="button-secondary" type="button" onClick={() => onNavigate("movements")}>Movimentações</button>
      </div>
    </section>
  ...
  return <div className="page-stack compact-stack">
    <section className="panel">
      ...
      <div className="summary-grid">
        ...
        <Metric label="Próxima manutenção" value={center.nextMaintenanceAt ? formatDateTime(center.nextMaintenanceAt) : "Não agendada"} />
        <Metric label="Fotos no contexto da caixa" value={String(photos.length)} />
      </div>
      <div className="workspace-actions">
        <button type="button" onClick={() => onNavigate("agenda")}>Abrir Agenda</button>
        <button className="button-secondary" type="button" onClick={() => onNavigate("assets")}>Manutenção</button>
        <button className="button-secondary" type="button" onClick={() => onNavigate("boxes")}>Todas as caixas</button>
      </div>
    </section>
    <TaskPanel tasks={tasks} onNavigate={onNavigate} />
    <section className="panel wide-list">
      ...
      {occupancies.length === 0 ? <div className="empty-list">Esta caixa ainda não possui ocupações registradas.</div> : <div className="table-wrap"><table className="data-table"><thead><tr><th>Colônia</th><th>Início</th><th>Fim</th><th>Motivo</th><th>Estado</th></tr></thead><tbody>{occupancies.map((item) => <tr key={item.id}><td><strong>{item.colonyCode}</strong></td><td>{formatDateTime(item.startedAt)}</td><td>{item.endedAt ? formatDateTime(item.endedAt) : "Atual"}</td><td>{item.reason || "—"}</td><td>{item.correctedAt ? "Corrigido" : "Original"}</td></tr>)}</tbody></table></div>}
    </section>
    <section className="panel">
      ...
      {photos.length === 0 ? <div className="empty-list">Nenhuma foto associada ao contexto histórico desta caixa.</div> : <div className="record-list">{photos.slice(0, 6).map((photo) => <article className="record-card" key={photo.id}><div className="record-title-row"><div><strong>{photo.originalName}</strong><span>{photo.colonyCode} · {formatDateTime(photo.capturedAt)}</span></div></div>{photo.notes && <p>{photo.notes}</p>}</article>)}</div>}
    </section>
  </div>;
}
...
export function MeliponaryOperationalCenter({ meliponaryId, onNavigate }: NavigateProps & { meliponaryId: string }) {
  ...
  return <div className="page-stack compact-stack">
    <section className="panel">
      ...
      <div className="workspace-actions">
        <button type="button" onClick={() => onNavigate("agenda")}>Abrir Agenda</button>
        <button className="button-secondary" type="button" onClick={() => onNavigate("alerts")}>Alertas</button>
        <button className="button-secondary" type="button" onClick={() => onNavigate("colonies")}>Colônias</button>
        <button className="button-secondary" type="button" onClick={() => onNavigate("boxes")}>Caixas</button>
      </div>
    </section>
    <TaskPanel tasks={tasks} onNavigate={onNavigate} />
    <AlertPanel alerts={alerts} onNavigate={onNavigate} />
...
function TaskPanel({ tasks, onNavigate }: { tasks: ScheduledTask[]; onNavigate: (view: View) => void }) {
  return <section className="panel wide-list">
    ...
    {tasks.length === 0 ? <div className="empty-list">Nenhum compromisso pendente neste contexto.</div> : <div className="table-wrap"><table className="data-table"><thead><tr><th>Quando</th><th>Tarefa</th><th>Tipo</th><th>Prioridade</th></tr></thead><tbody>{tasks.slice(0, 6).map((task) => <tr key={task.id} className={task.overdue ? "attention-row" : undefined}><td>{formatDateTime(task.scheduledFor)}</td><td><strong>{task.title}</strong></td><td>{taskTypeLabel(task.taskType)}</td><td>{priorityLabel(task.priority)}</td></tr>)}</tbody></table></div>}
    <div className="form-actions"><button className="button-secondary" type="button" onClick={() => onNavigate("agenda")}>Gerenciar na Agenda</button></div>
  </section>;
}

function AlertPanel({ alerts, onNavigate }: { alerts: Alert[]; onNavigate: (view: View) => void }) {
  return <section className="panel">
    ...
    {alerts.length === 0 ? <div className="empty-list">Nenhum alerta atual neste contexto.</div> : <div className="record-list">{alerts.slice(0, 5).map((alert) => <article className="record-card" key={alert.alertKey}><div className="record-title-row"><div><strong>{alert.title}</strong><span>{alert.dueAt ? formatDateTime(alert.dueAt) : alertTypeLabel(alert.alertType)}</span></div><span className={`badge severity-${alert.severity}`}>{severityLabel(alert.severity)}</span></div>{alert.details && <p>{alert.details}</p>}</article>)}</div>}
