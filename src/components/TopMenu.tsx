...
export function TopMenu({ theme, onThemeChange, sidebarCollapsed, onToggleSidebar, onNavigate, onRefresh, onOpenAbout }: TopMenuProps) {
  ...
  return (
    <div className="menu-bar" ref={rootRef}>
      ...
      <Menu label="Arquivo" open={open === "file"} onToggle={() => toggle("file")}>
        <MenuItem label="Backup" onClick={action(() => onNavigate("data"))} />
        <MenuItem label="Restaurar" onClick={action(() => onNavigate("data"))} />
        <MenuItem label="Exportar" onClick={action(() => onNavigate("data"))} />
        <MenuItem label="Relatórios" onClick={action(() => onNavigate("data"))} />
        <div className="menu-separator" />
        <MenuItem label="Sair" onClick={action(() => { void getCurrentWindow().close(); })} />
      </Menu>
      <Menu label="Editar" open={open === "edit"} onToggle={() => toggle("edit")}>
        <MenuItem label="Novo registro" disabled />
        <MenuItem label="Editar registro" disabled />
      </Menu>
      <Menu label="Exibir" open={open === "view"} onToggle={() => toggle("view")}>
        <MenuItem label="Tema claro" checked={theme === "light"} icon="sun" onClick={action(() => onThemeChange("light"))} />
        <MenuItem label="Tema escuro" checked={theme === "dark"} icon="moon" onClick={action(() => onThemeChange("dark"))} />
        <MenuItem label="Seguir sistema" checked={theme === "system"} icon="system" onClick={action(() => onThemeChange("system"))} />
        <div className="menu-separator" />
        <MenuItem label={sidebarCollapsed ? "Expandir sidebar" : "Recolher sidebar"} onClick={action(onToggleSidebar)} />
        <MenuItem label="Atualizar" icon="refresh" onClick={action(onRefresh)} />
      </Menu>
      <Menu label="Ferramentas" open={open === "tools"} onToggle={() => toggle("tools")}>
        <MenuItem label="Dados" icon="database" onClick={action(() => onNavigate("data"))} />
      </Menu>
      <Menu label="Ajuda" open={open === "help"} onToggle={() => toggle("help")}>
        <MenuItem label="Sobre" icon="info" onClick={action(onOpenAbout)} />
      </Menu>
    </div>
  );
}
