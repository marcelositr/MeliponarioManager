...
import type {
  Alert, Colony, ColonyEvent, CoreData, CoreSummary, CreateBoxInput, CreateColonyEventInput,
  CreateColonyInput, CreateFeedingInput, CreateInspectionInput, CreateMeliponaryInput,
  CreateProductionInput, CreateSpeciesInput, DashboardStats, Feeding, HiveBox, Inspection,
  Meliponary, PlaceColonyInput, ProductionRecord, Species, TimelineEntry,
} from "../types";

export async function loadCoreData(): Promise<CoreData> {
  const [meliponaries, species, colonies, boxes] = await Promise.all([
    invoke<Meliponary[]>("list_meliponaries"), invoke<Species[]>("list_species"),
    invoke<Colony[]>("list_colonies"), invoke<HiveBox[]>("list_boxes"),
  ]);
...
export async function loadDashboardStats(): Promise<DashboardStats> {
  const [summary, inspections, photos, events, divisions, feedings, production, movements, documents, maintenance, lifecycle, alerts] = await Promise.all([
    invoke<CoreSummary>("get_core_summary"), invoke<number>("get_inspection_count"), invoke<number>("get_inspection_photo_count"),
    invoke<number>("get_event_count"), invoke<number>("get_division_count"), invoke<number>("get_feeding_count"),
    invoke<number>("get_production_count"), invoke<number>("get_movement_count"), invoke<number>("get_movement_document_count"),
    invoke<number>("get_box_maintenance_count"), invoke<number>("get_lifecycle_count"), invoke<number>("get_alert_count"),
  ]);
