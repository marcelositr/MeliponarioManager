...
1. **ACH-001** — fechar a fronteira transacional entre fato/tarefa e reconciliação da Agenda.
...
- **Comportamento observado:** criação/correção/anulação de fatos pode concluir a gravação principal e somente depois chamar `agenda::reconcile_*`. Na execução especializada da Agenda, fato e conclusão da tarefa são commitados antes da reconciliação que cria/ajusta o próximo compromisso.
- **Risco:** uma falha de reconciliação pode retornar erro ao usuário mesmo com parte da operação já persistida. Em criação direta, uma tentativa repetida pode gerar outro fato. Na execução de tarefa, a tarefa pode já estar concluída quando a interface recebe falha.
- **Comportamento esperado:** a unidade de consistência prometida pelo fluxo deve ser atômica ou, se a reconciliação for deliberadamente eventual, uma falha pós-commit não pode ser apresentada como se a gravação principal tivesse falhado.
- **Contrato divergente:** `docs/AGENDA.md` afirma que fato, próximo compromisso derivado e conclusão da tarefa são confirmados na mesma transação e que nenhum estado parcial é promovido.
- **Correção:** a definir no início do bloco de implementação; preferência por reutilizar operações `*_tx`/reconciliação transacional sem criar abstração genérica desnecessária.
- **Teste de regressão:** obrigatório, incluindo falha induzida na etapa de Agenda ou verificação da mesma transação.
- **Commit/PR:** pendente.
...
Iniciar **ACH-001 — Fato pode ser persistido antes de a reconciliação da Agenda falhar**.

Antes de modificar o código:

1. mapear todos os pontos que chamam `agenda::reconcile_inspection`, `agenda::reconcile_feeding` e `agenda::reconcile_maintenance` depois de uma escrita;
2. definir a menor API transacional reutilizável necessária;
3. preservar o contrato IPC existente;
4. adicionar regressão que prove ausência de estado parcial;
5. corrigir ACH-001 sem misturar ACH-002 inteiro no mesmo patch, salvo quando a mesma primitiva transacional for necessária.
