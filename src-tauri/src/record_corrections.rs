...
const MAINTENANCE_TYPES: &[&str] = &[
    "cleaning", "repair", "painting", "waterproofing", "roof", "entrance",
    "internal_structure", "inspection", "other",
];
const EVENT_TYPES: &[&str] = &[
    "swarming", "abandonment", "queen_loss", "attack", "pest", "recovery",
    "maintenance", "observation", "other",
];
const SEVERITIES: &[&str] = &["info", "attention", "critical"];
const DOCUMENT_TYPES: &[&str] = &[
    "gta", "authorization", "invoice", "receipt", "declaration", "protocol",
    "certificate", "other",
];
...
pub async fn correct_inspection(pool: &SqlitePool, input: CorrectInspection) -> Result<(), AppError> {
    ...
    if !STRENGTHS.contains(&strength.as_str()) {
        return Err(AppError::Validation("Força da colônia inválida.".to_owned()));
    }
    let inspected_at = time::normalize(&input.inspected_at, false)?;
    let next = time::normalize_optional(&input.next_inspection_at, false)?;
    time::ensure_not_before(&next, &inspected_at, "A próxima inspeção não pode ser anterior à inspeção registrada.")?;

    let colony_id: Option<String> = sqlx::query_scalar("SELECT colony_id FROM inspections WHERE id = ? AND voided_at IS NULL")
        .bind(&id).fetch_optional(pool).await?;
    let colony_id = colony_id.ok_or_else(|| AppError::Validation("Inspeção não encontrada ou já anulada.".to_owned()))?;
    ...
    .bind(box_id).bind(&inspected_at).bind(strength).bind(input.queen_present)
    .bind(optional(&input.laying_status)).bind(optional(&input.food_reserves))
    .bind(optional(&input.brood_status)).bind(optional(&input.pests_notes))
    .bind(optional(&input.observations)).bind(optional(&input.actions_taken)).bind(next)
    .bind(corrected_at).bind(&id).execute(&mut *tx).await?;
    let after = snapshot_tx(&mut tx,
        "SELECT json_object('id',id,'colony_id',colony_id,'box_id',box_id,'inspected_at',inspected_at,'strength',strength,'queen_present',queen_present,'laying_status',laying_status,'food_reserves',food_reserves,'brood_status',brood_status,'pests_notes',pests_notes,'observations',observations,'actions_taken',actions_taken,'next_inspection_at',next_inspection_at,'voided_at',voided_at) FROM inspections WHERE id = ?",
        &id, "Inspeção não encontrada.").await?;
    audit::record_tx(&mut tx, "inspection", &id, "correct", &reason, Some(before), Some(after)).await?;
    tx.commit().await?;
    Ok(())
}
...
pub async fn correct_feeding(pool: &SqlitePool, input: CorrectFeeding) -> Result<(), AppError> {
    ...
    time::ensure_not_before(&next, &fed_at, "A próxima alimentação não pode ser anterior à alimentação registrada.")?;
    let unit = optional(&input.unit);
    match (input.quantity, unit.as_ref()) {
        (None, None) => {}
        (Some(value), Some(_)) if value.is_finite() && value > 0.0 => {}
        (Some(_), None) => return Err(AppError::Validation("Informe a unidade quando houver quantidade.".to_owned())),
        (None, Some(_)) => return Err(AppError::Validation("Informe a quantidade quando houver unidade.".to_owned())),
        _ => return Err(AppError::Validation("A quantidade precisa ser maior que zero.".to_owned())),
    }
    let colony_id: Option<String> = sqlx::query_scalar("SELECT colony_id FROM feedings WHERE id=? AND voided_at IS NULL")
        .bind(&id).fetch_optional(pool).await?;
    let colony_id = colony_id.ok_or_else(|| AppError::Validation("Alimentação não encontrada ou já anulada.".to_owned()))?;
    ...
    audit::record_tx(&mut tx, "feeding", &id, "correct", &reason, Some(before), Some(after)).await?;
    tx.commit().await?;
    Ok(())
}
...
pub async fn correct_production(pool: &SqlitePool, input: CorrectProduction) -> Result<(), AppError> {
    ...
    if !input.quantity.is_finite() || input.quantity <= 0.0 {
        return Err(AppError::Validation("A quantidade precisa ser maior que zero.".to_owned()));
    }
    let unit = required(&input.unit, "Unidade")?;
    let harvested_at = time::normalize(&input.harvested_at, false)?;
    let colony_id: Option<String> = sqlx::query_scalar("SELECT colony_id FROM production_records WHERE id=? AND voided_at IS NULL")
        .bind(&id).fetch_optional(pool).await?;
    let colony_id = colony_id.ok_or_else(|| AppError::Validation("Produção não encontrada ou já anulada.".to_owned()))?;
    ...
    audit::record_tx(&mut tx, "production", &id, "correct", &reason, Some(before), Some(after)).await?;
    tx.commit().await?;
    Ok(())
}
...
pub async fn correct_maintenance(pool: &SqlitePool, input: CorrectMaintenance) -> Result<(), AppError> {
    ...
    if input.cost.is_some_and(|value| !value.is_finite() || value < 0.0) {
        return Err(AppError::Validation("O custo precisa ser válido e não negativo.".to_owned()));
    }
    ...
    time::ensure_not_before(&next, &maintained_at, "A próxima manutenção não pode ser anterior à manutenção registrada.")?;
    let box_exists: bool = sqlx::query_scalar("SELECT EXISTS(SELECT 1 FROM boxes WHERE id=?)")
        .bind(&box_id).fetch_one(pool).await?;
    if !box_exists { return Err(AppError::NotFound("Caixa não encontrada.".to_owned())); }
    let exists: bool = sqlx::query_scalar("SELECT EXISTS(SELECT 1 FROM box_maintenance_records WHERE id=? AND voided_at IS NULL)")
        .bind(&id).fetch_one(pool).await?;
    if !exists { return Err(AppError::Validation("Manutenção não encontrada ou já anulada.".to_owned())); }
    ...
    audit::record_tx(&mut tx, "box_maintenance", &id, "correct", &reason, Some(before), Some(after)).await?;
    tx.commit().await?;
    Ok(())
}
...
pub async fn correct_event(pool: &SqlitePool, input: CorrectEvent) -> Result<(), AppError> {
    ...
    let reason = required(&input.reason, "Motivo da correção")?;
    if !EVENT_TYPES.contains(&event_type.as_str()) { return Err(AppError::Validation("Tipo de evento inválido.".to_owned())); }
    if !SEVERITIES.contains(&severity.as_str()) { return Err(AppError::Validation("Nível do evento inválido.".to_owned())); }
    let occurred_at = time::normalize(&input.occurred_at, false)?;
    let now = time::local_now(pool).await?;
    if occurred_at > now { return Err(AppError::Validation("Eventos manuais representam fatos ocorridos e não podem ser registrados no futuro. Use a Agenda em etapa própria.".to_owned())); }
    let colony_id: Option<String> = sqlx::query_scalar("SELECT colony_id FROM colony_events WHERE id=? AND voided_at IS NULL")
        .bind(&id).fetch_optional(pool).await?;
    let colony_id = colony_id.ok_or_else(|| AppError::Validation("Evento não encontrado ou já anulado.".to_owned()))?;
    ...
    audit::record_tx(&mut tx, "colony_event", &id, "correct", &reason, Some(before), Some(after)).await?;
    tx.commit().await?;
    Ok(())
}
...
pub async fn correct_movement_details(pool: &SqlitePool, input: CorrectMovementDetails) -> Result<(), AppError> {
    ...
    let (movement_type, voided_at, reversed_at) = movement.ok_or_else(|| AppError::NotFound("Movimentação não encontrada.".to_owned()))?;
    if voided_at.is_some() || reversed_at.is_some() { return Err(AppError::Validation("Movimentação anulada ou revertida não pode ser editada.".to_owned())); }
    let destination = optional(&input.destination);
    if movement_type != "internal_transfer" && destination.is_none() {
        return Err(AppError::Validation("Informe o destino da movimentação.".to_owned()));
    }
    let mut tx = pool.begin().await?;
    ...
    sqlx::query("UPDATE colony_movements SET destination=?, notes=?, corrected_at=? WHERE id=?")
        .bind(if movement_type == "internal_transfer" { None::<String> } else { destination })
        .bind(optional(&input.notes)).bind(corrected_at).bind(&id).execute(&mut *tx).await?;
    let after = snapshot_tx(&mut tx, snapshot_sql, &id, "Movimentação não encontrada.").await?;
    audit::record_tx(&mut tx, "movement", &id, "correct", &reason, Some(before), Some(after)).await?;
    tx.commit().await?;
    Ok(())
}
...
pub async fn void_transport(pool: &SqlitePool, input: VoidRecord) -> Result<(), AppError> {
    let id = required(&input.id, "Movimentação")?;
    let kind: Option<String> = sqlx::query_scalar("SELECT movement_type FROM colony_movements WHERE id=?")
        .bind(&id).fetch_optional(pool).await?;
    match kind.as_deref() {
...
pub async fn update_movement_document(pool: &SqlitePool, input: UpdateMovementDocument) -> Result<(), AppError> {
    ...
    if !DOCUMENT_TYPES.contains(&document_type.as_str()) { return Err(AppError::Validation("Tipo de documento inválido.".to_owned())); }
    ...
    if let (Some(issued), Some(valid)) = (&issued_at, &valid_until) {
        if valid < issued { return Err(AppError::Validation("A validade não pode ser anterior à emissão.".to_owned())); }
    }
    let movement_id: Option<String> = sqlx::query_scalar("SELECT movement_id FROM movement_documents WHERE id=? AND voided_at IS NULL")
        .bind(&id).fetch_optional(pool).await?;
    let movement_id = movement_id.ok_or_else(|| AppError::Validation("Documento não encontrado ou invalidado.".to_owned()))?;
    ...
    if duplicate { return Err(AppError::Validation("Já existe um documento válido com esta referência na movimentação.".to_owned())); }
    let mut tx = pool.begin().await?;
    ...
    audit::record_tx(&mut tx, "movement_document", &id, "edit", &reason, Some(before), Some(after)).await?;
    tx.commit().await?;
    Ok(())
}
...
pub async fn correct_division(pool: &SqlitePool, input: CorrectDivision) -> Result<(), AppError> {
    ...
    let already_void: Option<String> = sqlx::query_scalar("SELECT voided_at FROM colony_divisions WHERE id=?")
        .bind(&id).fetch_one(&mut *tx).await?;
    if already_void.is_some() { return Err(AppError::Validation("Divisão anulada não pode ser corrigida.".to_owned())); }
    let corrected_at = now_tx(&mut tx).await?;
    ...
    audit::record_tx(&mut tx, "division", &id, "correct", &reason, Some(before), Some(after)).await?;
    tx.commit().await?;
    Ok(())
}
...
pub async fn void_division(pool: &SqlitePool, input: VoidDivision) -> Result<(), AppError> {
    ...
    let row: Option<(Option<String>, Option<String>)> = sqlx::query_as("SELECT daughter_colony_id, voided_at FROM colony_divisions WHERE id=?")
        .bind(&id).fetch_optional(pool).await?;
    let (daughter_id, voided_at) = row.ok_or_else(|| AppError::NotFound("Divisão não encontrada.".to_owned()))?;
    if voided_at.is_some() { return Err(AppError::Validation("A divisão já está anulada.".to_owned())); }
    ...
    sqlx::query("UPDATE colony_divisions SET voided_at=?, void_reason=? WHERE id=?")
        .bind(&voided_at).bind(&reason).bind(&id).execute(&mut *tx).await?;
    ...
    if let (Some(daughter_id), Some("deactivate")) = (&daughter_id, disposition.as_deref()) {
        let previous: String = sqlx::query_scalar("SELECT status FROM colonies WHERE id=?")
            .bind(daughter_id).fetch_one(&mut *tx).await?;
        if matches!(previous.as_str(), "active" | "weak" | "recovering") {
            sqlx::query("UPDATE colonies SET status='inactive', updated_at=CURRENT_TIMESTAMP WHERE id=?")
                .bind(daughter_id).execute(&mut *tx).await?;
    ...
    let after = snapshot_tx(&mut tx, snapshot_sql, &id, "Divisão não encontrada.").await?;
    audit::record_tx(&mut tx, "division", &id, "void", &reason, Some(before), Some(after)).await?;
    tx.commit().await?;
    Ok(())
}
...
pub async fn correct_occupancy(pool: &SqlitePool, input: CorrectOccupancy) -> Result<(), AppError> {
    ...
    if ended_at.as_deref().is_some_and(|end| end < started_at.as_str()) {
        return Err(AppError::Validation("O fim da ocupação não pode ser anterior ao início.".to_owned()));
    }
    let row: Option<(String, String, String, Option<String>)> = sqlx::query_as(
        "SELECT colony_id, box_id, started_at, ended_at FROM colony_box_occupancies WHERE id=?",
    ).bind(&id).fetch_optional(pool).await?;
    let (colony_id, box_id, old_start, old_end) = row.ok_or_else(|| AppError::NotFound("Ocupação não encontrada.".to_owned()))?;
    ...
    ).bind(&box_id).bind(&started_at).fetch_optional(pool).await?;
    if state_at.as_deref().unwrap_or("active") != "active" {
        return Err(AppError::Validation("A caixa não estava ativa no novo início da ocupação.".to_owned()));
    }
    ...
    ).bind(&id).bind(&colony_id).bind(new_end_for_overlap).bind(&started_at).fetch_one(pool).await?;
    if colony_overlap { return Err(AppError::Validation("A correção faria a colônia ocupar duas caixas no mesmo intervalo.".to_owned())); }
    let box_overlap: bool = sqlx::query_scalar(
        "SELECT EXISTS(SELECT 1 FROM colony_box_occupancies
            WHERE id<>? AND box_id=? AND started_at<=?
              AND COALESCE(ended_at,'9999-12-31 23:59:59')>=?)",
    ).bind(&id).bind(&box_id).bind(new_end_for_overlap).bind(&started_at).fetch_one(pool).await?;
    if box_overlap { return Err(AppError::Validation("A correção faria a caixa abrigar duas colônias no mesmo intervalo.".to_owned())); }
    ...
    audit::record_tx(&mut tx, "box_occupancy", &id, "correct", &reason, Some(before), Some(after)).await?;
    tx.commit().await?;
    Ok(())
}
...
async fn void_fact(
    ...
    let current_void: Option<String> = sqlx::query_scalar(&format!("SELECT voided_at FROM {table} WHERE id=?"))
        .bind(&id).fetch_one(&mut *tx).await?;
    if current_void.is_some() { return Err(AppError::Validation("O registro já está anulado.".to_owned())); }
    let voided_at = now_tx(&mut tx).await?;
    sqlx::query(&format!("UPDATE {table} SET voided_at=?, void_reason=? WHERE id=?"))
        .bind(voided_at).bind(&reason).bind(&id).execute(&mut *tx).await?;
    let after = snapshot_tx(&mut tx, snapshot_sql, &id, "Registro não encontrado.").await?;
    audit::record_tx(&mut tx, entity_type, &id, "void", &reason, Some(before), Some(after)).await?;
    tx.commit().await?;
    Ok(())
}
...
mod tests {
    ...
    async fn seeded() -> (SqlitePool, String, String) {
        ...
        let box1 = repository::create_box(&pool, CreateHiveBox { meliponary_id:mel.id.clone(), code:"CX-001".into(), model:None, material:None, location_note:None, notes:None }).await.unwrap();
        let colony = repository::create_colony(&pool, CreateColony { meliponary_id:mel.id, species_id:species.id, code:"JAT-001".into(), origin_type:None, origin_notes:None, installed_at:Some("2026-01-01 09:00:00".into()), mother_colony_id:None, notes:None }).await.unwrap();
        repository::place_colony(&pool, PlaceColony { colony_id:colony.id.clone(), box_id:box1.id.clone(), started_at:Some("2026-01-01 09:00:00".into()), reason:None, notes:None }).await.unwrap();
    ...
    async fn inspection_correction_and_void_preserve_latest_valid_semantics() {
        ...
        let old = inspections::create(&pool, CreateInspection { colony_id:colony_id.clone(), inspected_at:Some("2026-01-10 10:00:00".into()), strength:Some("weak".into()), queen_present:None, laying_status:None, food_reserves:None, brood_status:None, pests_notes:None, observations:None, actions_taken:None, next_inspection_at:Some("2026-01-20 10:00:00".into()) }).await.unwrap();
        let latest = inspections::create(&pool, CreateInspection { colony_id:colony_id.clone(), inspected_at:Some("2026-02-10 10:00:00".into()), strength:Some("strong".into()), queen_present:None, laying_status:None, food_reserves:None, brood_status:None, pests_notes:None, observations:None, actions_taken:None, next_inspection_at:None }).await.unwrap();
        correct_inspection(&pool, CorrectInspection { id:old.id.clone(), inspected_at:"2026-01-11 10:00:00".into(), strength:"medium".into(), queen_present:None, laying_status:None, food_reserves:None, brood_status:None, pests_notes:None, observations:None, actions_taken:None, next_inspection_at:Some("2026-01-21 10:00:00".into()), reason:"Data corrigida".into() }).await.unwrap();
        void_inspection(&pool, VoidRecord { id:latest.id, reason:"Lançamento duplicado".into() }).await.unwrap();
        assert!(!alerts::list(&pool).await.unwrap().iter().any(|a| a.alert_type=="weak_colony"));
        let audit_count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM audit_records WHERE entity_type='inspection'").fetch_one(&pool).await.unwrap();
    ...
    async fn invalid_inspection_correction_date_is_rejected() {
        let (pool, colony_id, _) = seeded().await;
        let item = inspections::create(&pool, CreateInspection { colony_id, inspected_at:Some("2026-02-10 10:00:00".into()), strength:Some("strong".into()), queen_present:None, laying_status:None, food_reserves:None, brood_status:None, pests_notes:None, observations:None, actions_taken:None, next_inspection_at:None }).await.unwrap();
        assert!(correct_inspection(&pool, CorrectInspection { id:item.id, inspected_at:"2025-01-01 10:00:00".into(), strength:"strong".into(), queen_present:None, laying_status:None, food_reserves:None, brood_status:None, pests_notes:None, observations:None, actions_taken:None, next_inspection_at:None, reason:"Teste".into() }).await.is_err());
    }

    #[tokio::test]
    async fn voided_feeding_does_not_generate_due_alert() {
        let (pool, colony_id, _) = seeded().await;
        let item = feeding::create(&pool, CreateFeeding { colony_id, fed_at:Some("2026-01-10 10:00:00".into()), food_type:"Xarope".into(), quantity:Some(10.0), unit:Some("ml".into()), response_notes:None, notes:None, next_feeding_at:Some("2026-01-11 10:00:00".into()) }).await.unwrap();
        void_feeding(&pool, VoidRecord { id:item.id, reason:"Erro de lançamento".into() }).await.unwrap();
        assert!(!alerts::list(&pool).await.unwrap().iter().any(|a| a.alert_type=="feeding_due"));
    }

    #[tokio::test]
    async fn voided_production_is_excluded_from_valid_count() {
        let (pool, colony_id, _) = seeded().await;
        let item = production::create(&pool, CreateProductionRecord { colony_id, harvested_at:Some("2026-02-01 10:00:00".into()), product_type:"honey".into(), quantity:20.0, unit:"ml".into(), purpose:None, notes:None }).await.unwrap();
        void_production(&pool, VoidRecord { id:item.id, reason:"Pesagem incorreta".into() }).await.unwrap();
    ...
    async fn maintenance_correction_revalidates_next_date() {
        ...
        let item = maintenance::create(&pool, CreateBoxMaintenance { box_id:box_id.clone(), maintained_at:Some("2026-02-01 10:00:00".into()), maintenance_type:"repair".into(), description:None, performed_by:None, cost:None, next_maintenance_at:None }).await.unwrap();
        assert!(correct_maintenance(&pool, CorrectMaintenance { id:item.id, box_id, maintained_at:"2026-02-01 10:00:00".into(), maintenance_type:"repair".into(), description:None, performed_by:None, cost:None, next_maintenance_at:Some("2026-01-01 10:00:00".into()), reason:"Teste".into() }).await.is_err());
    }

    #[tokio::test]
    async fn occupancy_overlap_is_rejected() {
        let (pool, colony_id, first_box) = seeded().await;
        let mel_id: String = sqlx::query_scalar("SELECT meliponary_id FROM colonies WHERE id=?").bind(&colony_id).fetch_one(&pool).await.unwrap();
        let second = repository::create_box(&pool, CreateHiveBox { meliponary_id:mel_id, code:"CX-002".into(), model:None, material:None, location_note:None, notes:None }).await.unwrap();
        repository::place_colony(&pool, PlaceColony { colony_id:colony_id.clone(), box_id:second.id, started_at:Some("2026-03-01 10:00:00".into()), reason:None, notes:None }).await.unwrap();
        let first_id: String = sqlx::query_scalar("SELECT id FROM colony_box_occupancies WHERE colony_id=? AND box_id=?").bind(&colony_id).bind(first_box).fetch_one(&pool).await.unwrap();
        assert!(correct_occupancy(&pool, CorrectOccupancy { id:first_id, started_at:"2026-01-01 09:00:00".into(), ended_at:Some("2026-04-01 10:00:00".into()), occupancy_reason:None, notes:None, reason:"Teste".into() }).await.is_err());
    }
}
