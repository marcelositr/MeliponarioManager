...
pub async fn ensure_colony_available_at(
    ...
    .fetch_optional(pool)
    .await?;
    let (entry_at, current_status) = colony
    ...
    let effective_status = match status_at {
        ...
        None => {
            ...
            .fetch_one(pool)
            .await?;
            if later { "active".to_owned() } else { current_status }
        }
    };
...
mod tests {
    ...
    async fn seed_colony(pool: &SqlitePool, status: &str) -> String {
        sqlx::query("INSERT INTO meliponaries (id, name) VALUES ('m1', 'Principal')").execute(pool).await.unwrap();
        sqlx::query("INSERT INTO species (id, common_name) VALUES ('s1', 'Jataí')").execute(pool).await.unwrap();
    ...
    async fn active_and_legacy_manageable_statuses_are_available() {
        for status in ["active", "weak", "recovering"] {
            let pool=pool().await;let id=seed_colony(&pool,status).await;
            ensure_colony_available_at(&pool,&id,"2026-02-01 10:00:00").await.unwrap();
        }
    }
    #[tokio::test]
    async fn retrospective_fact_before_loss_is_allowed_but_later_fact_is_rejected() {
        let pool=pool().await;let id=seed_colony(&pool,"lost").await;
        ...
        ensure_colony_available_at(&pool,&id,"2026-02-20 10:00:00").await.unwrap();
        assert!(ensure_colony_available_at(&pool,&id,"2026-03-11 10:00:00").await.is_err());
    }
    #[tokio::test]
    async fn inactivity_interval_is_rejected_and_reactivation_restores_availability() {
        let pool=pool().await;let id=seed_colony(&pool,"active").await;
        ...
        assert!(ensure_colony_available_at(&pool,&id,"2026-03-15 10:00:00").await.is_err());
        ensure_colony_available_at(&pool,&id,"2026-04-02 10:00:00").await.unwrap();
    }
    #[tokio::test]
    async fn external_transfer_blocks_later_management_but_preserves_retrospective_facts() {
        let pool=pool().await;let id=seed_colony(&pool,"transferred").await;
        ...
        ensure_colony_available_at(&pool,&id,"2026-04-30 10:00:00").await.unwrap();
        assert!(ensure_colony_available_at(&pool,&id,"2026-05-02 10:00:00").await.is_err());
    }
    #[tokio::test]
    async fn reversed_external_transfer_no_longer_blocks_management() {
        ...
        ensure_colony_available_at(&pool,&id,"2026-05-03 10:00:00").await.unwrap();
    }
}
