...
pub async fn ensure_colony_available_at(
    ...
    let colony: Option<(String, String)> = sqlx::query_as(
        "SELECT COALESCE(installed_at, created_at), status
         FROM colonies
         WHERE id = ?",
    )
    .bind(colony_id)
    ...
    let external_transfer: bool = sqlx::query_scalar(
        "SELECT EXISTS(
            ...
            WHERE colony_id = ?
              AND movement_type = 'external_transfer'
              AND moved_at <= ?
        )",
    )
    .bind(colony_id)
    ...
    let status_at: Option<String> = sqlx::query_scalar(
        "SELECT new_status
         ...
         WHERE colony_id = ? AND occurred_at <= ?
...
mod tests {
    ...
    async fn seed_colony(pool: &SqlitePool, status: &str) -> String {
        ...
        sqlx::query(
            "INSERT INTO colonies (
                id, meliponary_id, species_id, code, installed_at, status
    ...
    async fn active_and_legacy_manageable_statuses_are_available() {
        for status in ["active", "weak", "recovering"] {
            let pool = pool().await;
            let colony_id = seed_colony(&pool, status).await;
            ensure_colony_available_at(&pool, &colony_id, "2026-02-01 10:00:00")
                .await
                .unwrap();
        }
    }
    ...
    async fn retrospective_fact_before_loss_is_allowed_but_later_fact_is_rejected() {
        let pool = pool().await;
        let colony_id = seed_colony(&pool, "lost").await;
        sqlx::query(
            "INSERT INTO colony_lifecycle_records (
                id, colony_id, action, occurred_at, previous_status, new_status
             ) VALUES ('life1', ?, 'loss', '2026-03-10 10:00:00', 'active', 'lost')",
        )
        .bind(&colony_id)
        .execute(&pool)
        .await
    ...
    async fn inactivity_interval_is_rejected_and_reactivation_restores_availability() {
        let pool = pool().await;
        let colony_id = seed_colony(&pool, "active").await;
        sqlx::query(
            "INSERT INTO colony_lifecycle_records (
                id, colony_id, action, occurred_at, previous_status, new_status
             ) VALUES
                ('life1', ?, 'deactivate', '2026-03-01 10:00:00', 'active', 'inactive'),
                ('life2', ?, 'reactivate', '2026-04-01 10:00:00', 'inactive', 'active')",
        )
        .bind(&colony_id)
        .bind(&colony_id)
        .execute(&pool)
    ...
    async fn external_transfer_blocks_later_management_but_preserves_retrospective_facts() {
        ...
        sqlx::query(
            "INSERT INTO colony_movements (
                id, colony_id, movement_type, moved_at, from_meliponary_id, destination
             ) VALUES ('move1', ?, 'external_transfer', '2026-05-01 10:00:00', 'm1', 'Outro criador')",
        )
        .bind(&colony_id)
        .execute(&pool)
        .await
