...
fn copy_tree(source: &Path, target: &Path) -> Result<(), String> {
    ...
    fs::create_dir_all(target).map_err(|error| io_error("Não foi possível criar diretório", error))?;
    for entry in fs::read_dir(source).map_err(|error| io_error("Não foi possível ler diretório", error))? {
        let entry = entry.map_err(|error| io_error("Não foi possível ler item", error))?;
        let source_path = entry.path();
        let target_path = target.join(entry.file_name());
        if entry.file_type().map_err(|error| io_error("Não foi possível identificar item", error))?.is_dir() {
            copy_tree(&source_path, &target_path)?;
        } else {
            fs::copy(&source_path, &target_path).map_err(|error| io_error("Não foi possível copiar arquivo", error))?;
        }
    }
    Ok(())
}
...
pub fn apply_pending_restore(data_dir: &Path) -> Result<(), String> {
    ...
    fs::create_dir_all(&safety_dir).map_err(|error| io_error("Não foi possível criar backup de segurança", error))?;
    ...
    if current_db.exists() {
        fs::remove_file(&current_db).map_err(|error| io_error("Não foi possível substituir o banco atual", error))?;
    }
    match fs::rename(&pending_db, &current_db) {
        Ok(()) => {}
        Err(_) => {
            fs::copy(&pending_db, &current_db).map_err(|error| io_error("Não foi possível aplicar o banco restaurado", error))?;
            fs::remove_file(&pending_db).map_err(|error| io_error("Não foi possível limpar o banco temporário", error))?;
        }
    }
    ...
    if pending_media.exists() {
        if current_media.exists() {
            fs::remove_dir_all(&current_media).map_err(|error| io_error("Não foi possível substituir a mídia atual", error))?;
        }
        match fs::rename(&pending_media, &current_media) {
            Ok(()) => {}
            Err(_) => {
                copy_tree(&pending_media, &current_media)?;
                fs::remove_dir_all(&pending_media).map_err(|error| io_error("Não foi possível limpar mídia temporária", error))?;
            }
        }
    }
...
#[tauri::command]
pub async fn create_full_backup(app: AppHandle, pool: State<'_, SqlitePool>) -> Result<GeneratedArtifact, String> {
    ...
    Ok(GeneratedArtifact { kind: "backup".into(), path: target_dir.to_string_lossy().into_owned(), created_at })
}
...
async fn validate_database(path: &Path) -> Result<(), String> {
    ...
    let pool = SqlitePoolOptions::new().max_connections(1).connect_with(options).await.map_err(|error| format!("Não foi possível abrir o banco do backup: {error}"))?;
    let integrity: String = sqlx::query_scalar("PRAGMA integrity_check").fetch_one(&pool).await.map_err(|error| error.to_string())?;
    if integrity.to_ascii_lowercase() != "ok" {
        pool.close().await;
        return Err(format!("Falha na verificação de integridade do backup: {integrity}"));
    }
    let has_core: bool = sqlx::query_scalar("SELECT EXISTS(SELECT 1 FROM sqlite_master WHERE type='table' AND name='colonies')")
...
#[tauri::command]
pub async fn stage_restore(app: AppHandle, backup_path: String) -> Result<RestoreStageResult, String> {
    let trimmed = backup_path.trim();
    if trimmed.is_empty() {
        return Err("Informe o diretório do backup ou o arquivo meliponario.db.".into());
    }
    let source = PathBuf::from(trimmed);
    let source_db = if source.is_dir() { source.join("meliponario.db") } else { source.clone() };
    validate_database(&source_db).await?;

    let data_dir = app.path().app_data_dir().map_err(|error| error.to_string())?;
    fs::create_dir_all(&data_dir).map_err(|error| io_error("Não foi possível acessar os dados da aplicação", error))?;
    ...
    let pending_media = data_dir.join("restore.pending-media");
    if pending_media.exists() {
        fs::remove_dir_all(&pending_media).map_err(|error| io_error("Não foi possível limpar mídia pendente", error))?;
    }
...
#[tauri::command]
pub async fn export_portable_json(app: AppHandle, pool: State<'_, SqlitePool>) -> Result<GeneratedArtifact, String> {
    let data_dir = app.path().app_data_dir().map_err(|error| error.to_string())?;
    let created_at = timestamp(&pool).await?;
    let export_dir = data_dir.join("exports");
    fs::create_dir_all(&export_dir).map_err(|error| io_error("Não foi possível criar diretório de exportação", error))?;

    let meliponaries = repository::list_meliponaries(&pool).await.map_err(|error| error.to_string())?;
    let species = repository::list_species(&pool).await.map_err(|error| error.to_string())?;
    let boxes = repository::list_boxes(&pool).await.map_err(|error| error.to_string())?;
    ...
    for colony in &colonies {
        ...
        timeline_by_colony.push(ColonyTimelineExport { colony_id: colony.id.clone(), colony_code: colony.code.clone(), timeline });
    }

    let export = PortableExport { generated_at: created_at.clone(), meliponaries, species, boxes, colonies, occupancies, timeline_by_colony };
    ...
    fs::write(&path, bytes).map_err(|error| io_error("Não foi possível gravar a exportação", error))?;
    Ok(GeneratedArtifact { kind: "json".into(), path: path.to_string_lossy().into_owned(), created_at })
}

#[tauri::command]
pub async fn generate_management_report(app: AppHandle, pool: State<'_, SqlitePool>) -> Result<GeneratedArtifact, String> {
    let data_dir = app.path().app_data_dir().map_err(|error| error.to_string())?;
    let created_at = timestamp(&pool).await?;
    let export_dir = data_dir.join("exports");
    fs::create_dir_all(&export_dir).map_err(|error| io_error("Não foi possível criar diretório de relatórios", error))?;
    let summary = repository::core_summary(&pool).await.map_err(|error| error.to_string())?;
    let overview = dashboard::overview(&pool).await.map_err(|error| error.to_string())?;
    ...
    for item in &overview.colony_statuses { report.push_str(&format!("- {}: {}\n", item.label, item.count)); }
    report.push_str("\n## Distribuição por espécie\n");
    for item in &overview.species_distribution { report.push_str(&format!("- {}: {}\n", item.label, item.count)); }
    report.push_str(&format!("\n## Pendências\n\nAlertas atuais: {}\n", overview.alerts.len()));
    for alert in overview.alerts.iter().take(20) { report.push_str(&format!("- {}: {}{}\n", alert.colony_code, alert.title, alert.due_at.as_ref().map(|value| format!(" ({value})")).unwrap_or_default())); }

    let path = export_dir.join(format!("relatorio-{created_at}.md"));
    fs::write(&path, report).map_err(|error| io_error("Não foi possível gravar o relatório", error))?;
    Ok(GeneratedArtifact { kind: "report".into(), path: path.to_string_lossy().into_owned(), created_at })
}
...
mod tests {
    ...
    async fn validates_migrated_database_for_restore() {
        let root = std::env::temp_dir().join(format!("meliponariomanager-restore-{}", Uuid::new_v4()));
