...
async fn v071_upgrade_preserves_operational_history_and_adds_box_state_integrity() {
    ...
    apply(&pool, include_str!("../../migrations/0013_box_state_integrity.sql")).await;

    assert_eq!(sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM colonies")
        .fetch_one(&pool).await.unwrap(), 1);
    assert_eq!(sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM colony_box_occupancies")
        .fetch_one(&pool).await.unwrap(), 1);
    assert_eq!(sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM inspection_photos")
        .fetch_one(&pool).await.unwrap(), 1);
    assert_eq!(sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM movement_documents")
        .fetch_one(&pool).await.unwrap(), 1);
    assert_eq!(sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM colony_events")
        .fetch_one(&pool).await.unwrap(), 1);
