...
pub(super) async fn colony_report(
    ...
    if filter
        .meliponary_id
        .as_deref()
        .is_some_and(|id| id != colony_meliponary_id(pool, colony_id).await.as_deref().unwrap_or(""))
    {
        return Err(AppError::Validation(
...
async fn colony_meliponary_id(pool: &SqlitePool, colony_id: &str) -> Option<String> {
    sqlx::query_scalar::<_, String>("SELECT meliponary_id FROM colonies WHERE id = ?")
        .bind(colony_id)
        .fetch_optional(pool)
        .await
        .ok()
...
async fn history_rows(
    ...
    Ok(sqlx::query_as::<_, ColonyHistoryRow>(
        "SELECT source_id, occurred_at, category, title, details, state
         FROM (
            SELECT o.id AS source_id, o.started_at AS occurred_at,
                   'Ocupação' AS category,
                   COALESCE(NULLIF(TRIM(o.reason), ''), 'Colônia colocada em caixa') AS title,
                   CASE WHEN b.code IS NULL THEN o.notes
                        WHEN o.notes IS NULL THEN 'Caixa ' || b.code
                        ELSE 'Caixa ' || b.code || ' · ' || o.notes END AS details,
                   CASE WHEN o.corrected_at IS NOT NULL THEN 'corrected' ELSE 'effective' END AS state
            FROM colony_box_occupancies o
            LEFT JOIN boxes b ON b.id = o.box_id
            WHERE o.colony_id = ?

            UNION ALL
            SELECT i.id, i.inspected_at, 'Inspeção', 'Inspeção',
                   COALESCE(i.observations, i.actions_taken),
                   CASE WHEN i.voided_at IS NOT NULL THEN 'voided'
                        WHEN i.corrected_at IS NOT NULL THEN 'corrected' ELSE 'effective' END
            FROM inspections i WHERE i.colony_id = ?

            UNION ALL
            SELECT f.id, f.fed_at, 'Alimentação', 'Alimentação: ' || f.food_type,
                   COALESCE(f.response_notes, f.notes),
                   CASE WHEN f.voided_at IS NOT NULL THEN 'voided'
                        WHEN f.corrected_at IS NOT NULL THEN 'corrected' ELSE 'effective' END
            FROM feedings f WHERE f.colony_id = ?

            UNION ALL
            SELECT p.id, p.harvested_at, 'Produção',
                   'Produção: ' || CASE p.product_type
                     WHEN 'honey' THEN 'Mel' WHEN 'pollen' THEN 'Pólen'
                     WHEN 'propolis' THEN 'Própolis' WHEN 'wax' THEN 'Cera'
                     WHEN 'cerumen' THEN 'Cerume' ELSE 'Outro produto' END,
                   printf('%g %s%s', p.quantity, p.unit,
                     CASE WHEN p.notes IS NOT NULL THEN ' · ' || p.notes ELSE '' END),
                   CASE WHEN p.voided_at IS NOT NULL THEN 'voided'
                        WHEN p.corrected_at IS NOT NULL THEN 'corrected' ELSE 'effective' END
            ...
            SELECT e.id, e.occurred_at, 'Evento', COALESCE(e.title, 'Evento operacional'), e.details,
                   CASE WHEN e.voided_at IS NOT NULL THEN 'voided'
                        WHEN e.corrected_at IS NOT NULL THEN 'corrected' ELSE 'effective' END
            FROM colony_events e WHERE e.colony_id = ?

            UNION ALL
            SELECT r.id, r.maintained_at, 'Manutenção', 'Manutenção de caixa', r.description,
                   CASE WHEN r.voided_at IS NOT NULL THEN 'voided'
                        WHEN r.corrected_at IS NOT NULL THEN 'corrected' ELSE 'effective' END
            FROM box_maintenance_records r WHERE r.colony_id = ?

            UNION ALL
            SELECT d.id, d.performed_at, 'Divisão', 'Divisão de colônia', d.notes,
                   CASE WHEN d.voided_at IS NOT NULL THEN 'voided'
                        WHEN d.corrected_at IS NOT NULL THEN 'corrected' ELSE 'effective' END
            ...
            SELECT m.id, m.moved_at, 'Movimentação',
                   CASE m.movement_type
                     WHEN 'internal_transfer' THEN 'Transferência interna'
                     WHEN 'external_transfer' THEN 'Transferência externa'
                     ELSE 'Transporte temporário' END,
                   COALESCE(dest.name, m.destination, m.notes),
                   CASE WHEN m.reversed_at IS NOT NULL THEN 'reversed'
                        WHEN m.voided_at IS NOT NULL THEN 'voided'
                        WHEN m.corrected_at IS NOT NULL THEN 'corrected' ELSE 'effective' END
            FROM colony_movements m
            LEFT JOIN meliponaries dest ON dest.id = m.to_meliponary_id
            WHERE m.colony_id = ?

            UNION ALL
            SELECT l.id, l.occurred_at, 'Ciclo de vida',
                   'Ciclo de vida: ' || l.previous_status || ' → ' || l.new_status,
                   COALESCE(l.reason, l.notes),
            ...
            FROM transport_returns tr
            JOIN colony_movements m ON m.id = tr.movement_id
            WHERE m.colony_id = ?
...
async fn append_audit_rows(
    ...
    let audits = sqlx::query_as::<_, AuditRow>(
        "SELECT id, entity_id, changed_at, action, reason
         FROM audit_records
