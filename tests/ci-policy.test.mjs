...
test("branch CI keeps fast feedback while PR and main retain full validation", async () => {
  ...
  assert.match(branchCi, /- name: Select validation profile/);
  assert.match(branchCi, /GITHUB_EVENT_NAME.*push/s);
  assert.match(branchCi, /GITHUB_REF_NAME.*work\/field-testing-and-hardening/s);
  assert.match(branchCi, /echo "relaxed=true"/);
  assert.match(branchCi, /CI profile: field-testing light/);
  ...
  assert.match(
    branchCi,
    /if: steps\.change_scope\.outputs\.code_changed == 'true' && steps\.validation_profile\.outputs\.relaxed != 'true'/,
  );
